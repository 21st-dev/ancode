...
const PROTOCOL = IS_DEV ? "crunch-dev" : "crunch"
...
export function getAppUrl(): string {
  return process.env.ELECTRON_RENDERER_URL || "https://getcrunch.dev/agents"
}
...
function handleDeepLink(url: string): void {
  ...
  try {
    ...
    // Handle auth callback: crunch://auth?code=xxx
    ...
    // Handle MCP OAuth callback: crunch://mcp-oauth?code=xxx&state=yyy
...
<head>
  ...
  <title>Crunch - Authentication</title>
  <style>
...
<head>
  <meta charset="UTF-8">
  <link rel="icon" type="image/svg+xml" href="${FAVICON_DATA_URI}">
  <title>Crunch - MCP Authentication</title>
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    :root {
      --bg: #09090b;
...
if (gotTheLock) {
  ...
  app.whenReady().then(async () => {
    ...
    if (process.platform === "win32") {
      app.setAppUserModelId(IS_DEV ? "dev.getcrunch.app.dev" : "dev.getcrunch.app")
    }

    console.log(`[App] Starting Crunch${IS_DEV ? " (DEV)" : ""}...`)
    ...
    app.setAboutPanelOptions({
      ...
      copyright: "Copyright © 2026 getcrunch.dev",
    })
          ...
          submenu: [
            ...
            { type: "separator" },
            {
              label: isCliInstalled()
                ? "Uninstall 'crunch' Command..."
                : "Install 'crunch' Command in PATH...",
              click: async () => {
                ...
                if (isCliInstalled()) {
                  ...
                  if (result.success) {
                    dialog.showMessageBox({
                      ...
                      detail: "The 'crunch' command has been removed from your PATH.",
                    })
                    buildMenu()
                  } else {
                  ...
                  if (result.success) {
                    dialog.showMessageBox({
                      ...
                      detail:
                        "You can now use 'crunch .' in any terminal to open Crunch in that directory.",
                    })
                    buildMenu()
                  } else {
          ...
          submenu: [
            {
              ...
              click: async () => {
                ...
                await shell.openExternal("https://getcrunch.dev")
              },
            },
          ],
        },
    ...
    // Handle directory argument from CLI (e.g., `crunch /path/to/project`)
    parseLaunchDirectory()
