...
export type WorktreeConfigSource = "custom" | "cursor" | "crunch" | null
...
const CRUNCH_CONFIG_PATH = ".crunch/worktree.json"
 ...
 * Priority: custom path > .cursor/worktrees.json > .crunch/worktree.json
 */
export async function detectWorktreeConfig(
  ...
  // 3. Check .crunch/worktree.json
  const crunchPath = join(projectPath, CRUNCH_CONFIG_PATH)
  if (await fileExists(crunchPath)) {
    const config = await readJsonFile<WorktreeConfig>(crunchPath)
    if (config) {
      return { config, path: crunchPath, source: "crunch" }
    }
  }
...
export async function getAvailableConfigPaths(
  ...
  crunch: { exists: boolean; path: string }
}> {
  const cursorPath = join(projectPath, CURSOR_CONFIG_PATH)
  const crunchPath = join(projectPath, CRUNCH_CONFIG_PATH)
  ...
  return {
    ...
    crunch: {
      exists: await fileExists(crunchPath),
      path: crunchPath,
    },
  }
}
...
export async function saveWorktreeConfig(
  ...
  target: "cursor" | "crunch" | string = "crunch",
  ...
  if (target === "cursor") {
    ...
    targetPath = join(projectPath, CRUNCH_CONFIG_PATH)
  } else {
    // Custom path
