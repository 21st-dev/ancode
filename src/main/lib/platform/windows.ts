...
export class WindowsPlatformProvider extends BasePlatformProvider {
  ...
  getCliConfig(): CliConfig {
    ...
    return {
      installPath: path.join(home, ".local", "bin", "crunch.cmd"),
      scriptName: "crunch.cmd",
  ...
  async installCli(
    ...
    try {
      ...
      console.log("[CLI] Installed crunch command to", installPath)
      console.log(
      ...
      return {
        success: true,
        pathHint: `To use crunch from terminal, add to your PATH: ${installDir}`,
      }
    } catch (error: unknown) {
      const errorMessage =
  ...
  async uninstallCli(): Promise<{ success: boolean; error?: string }> {
    ...
    try {
      ...
      console.log("[CLI] Uninstalled crunch command")
      return { success: true }
    } catch (error: unknown) {
      const errorMessage =
