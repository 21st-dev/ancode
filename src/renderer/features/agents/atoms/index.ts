...
// Derived atom for current project's model preference
// Returns { providerId, modelId } if project has preferences set, null otherwise
export const currentProjectModelPreferenceAtom = atom((get) => {
  const project = get(selectedProjectAtom)
  if (!project?.preferredProviderId || !project?.preferredModelId) {
    return null
  }
  return {
    providerId: project.preferredProviderId,
    modelId: project.preferredModelId,
  }
})
...
// All provider models (fetched from backend)
export const providerModelsAtom = atom<ProviderWithModels[]>([])

// Selected model per provider (persisted)
// Maps providerId -> modelId
export const selectedModelPerProviderAtom = atomWithStorage<Record<string, string>>(
  "agents:selectedModelPerProvider",
  {},
  undefined,
  { getOnInit: true },
)

// Helper to get selected model for a provider (with fallback to default)
export const getModelForProvider = (
  providerId: string,
  providersWithModels: ProviderWithModels[],
  selections: Record<string, string>,
): ProviderModel | null => {
  const provider = providersWithModels.find((p) => p.provider.id === providerId)
  if (!provider || provider.models.length === 0) return null

  // Try selected model first
  const selectedId = selections[providerId]
  if (selectedId) {
    const selected = provider.models.find((m) => m.modelId === selectedId)
...
export interface FetchModelsState {
  isLoading: boolean
  error: string | null
  lastFetched: number | null
}

export const fetchModelsStateAtom = atom<Record<string, FetchModelsState>>({})
